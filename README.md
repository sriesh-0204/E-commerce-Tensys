# Tensys-Ecommerce - Task 
## (submitted on 10-July-2024)

A simple web app built with React JS to have an authentication system & hosting powered by Firebase, home screen

![image description](data:image/png;base64,)

#### Product List Page Image:
![image description](https://github.com/sriesh-0204/tensys-task/assets/61575809/7504ca91-fa9d-4adc-b067-9cdc8e59f53c)
)

#### Product Detail Page Image:
![image description](https://github.com/sriesh-0204/tensys-task/assets/61575809/1db09c36-5b29-4818-952e-cd402040e79f)
<- Back:-Click Back button again list of product will show

#### Product List Search Filter Image:
![image description](https://github.com/sriesh-0204/E-commerce-Tensys/assets/61575809/6117610e-7de7-4ba0-8b42-a50b4aea6791)
Input filed type product name but that product list is not available no data image message will show

#### Product Detail Pagess Image:
![image description](https://github.com/sriesh-0204/E-commerce-Tensys/assets/61575809/74c78b42-6b3a-413f-bc1c-098beb8bf3ab)
<- Back:-Click Back button again list of product will show

## Tech Stack
| Library | Version  |
| :---:   | :---: |
| React | 18.2   |
| Sass | 1.72.x   |
| Firebase | 10.9.x   |






## Installation
Assuming you're all the pre requisites installed on your machine such as NodeJS, yarn or npm package manager.

#### Clone the repo

bash
  git clone https://github.com/sriesh-0204/E-commerce-Tensys.git
  cd Tensys-Ecommerce-Task
  
#### Live Link: 
    live link https://668e32fb3efc2c8d2675af29--graceful-fenglisu-59f41d.netlify.app/


#### Install dependenciesa

bash
  yarn (or) npm i
  
#### Start Server

npm run start
 


#### Start the server locally on your machine

bash
  yarn start


## Future enhancements

- We can add the following to the project:-
    - Add cart Item
    - Tailwind for 100% RWD 
    - eslint
    - Loaders & toast messages to indicate the app state
    - folder structure and components
